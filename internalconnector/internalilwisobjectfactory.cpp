...
bool InternalIlwisObjectFactory::createCoverage(const Resource& item, Coverage *coverage) const {
    ...
    coverage->setName(QString("%1_%2").arg(INTERNAL_PREFIX).arg(coverage->id()));
...
IlwisObject *InternalIlwisObjectFactory::createGridCoverage(const Resource& item) const {
    ...
    if (QString(item["georeference"].typeName()) == "Ilwis::IGeoReference")
        ...
        if ( bounds.isValid() && !bounds.isNull()){
            CornersGeoReference *cgeoref = new CornersGeoReference();
            cgeoref->setName("subset_" + gcoverage->name());
            cgeoref->coordinateSystem(gcoverage->coordinateSystem());
            cgeoref->setEnvelope(bounds);
            cgeoref->size(sz);
            grf.set(static_cast<GeoReference *>(cgeoref));
            if (!grf->compute()){
