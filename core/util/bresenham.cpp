...
<<<<<<< HEAD
        ...
        std::vector<Pixel> line  = makePixelLineIlw3(crd1, crd2, _valid, sz, subcount);
=======
        std::vector<Pixel> line  = makePixelLine(crd1, crd2, _valid, sz, subcount);
>>>>>>> 53646b951256160897b6549fb55f85903931c476
        if (!_valid)
...
std::vector<Pixel> Bresenham::makePixelLineIlw3(const Coordinate& crdStart, const Coordinate& crdEnd, bool& valid, const Size <> & sz, quint32 subcount) const
{
    valid = false;
    std::vector<Pixel> result;
    if ( !_targetGrf.isValid()){
