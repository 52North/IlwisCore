...
@init{
	...
	:	statement				 { node->addChild($statement.node); } EOL
	|	variable 				 { node->addChild($variable.node); } EOL
	|	constant				 { node->addChild($constant.node); } EOL
	|	function 				 { node->addChild($function.node); } EOL
	| 	EOL
	;
...
typeModifierPart returns [ ASTNode *node]
@init{
	node = new ASTNodeNode("typemodifier");
}
	:	'{' modifierPart'}'			{ node->addChild($modifierPart.node);}
	;

modifierPart returns [ ASTNode *node]
@init{
...
@init{
	...
	: 'format' '=' ID ':' STRING		{ node->setFormatNameSpace(new IDNode((char *)($ID.text->chars)));node->setFormat((char *)($text->chars));}
	| 'format' '=' STRING			{ node->setFormat((char *)($text->chars)); }
	;
